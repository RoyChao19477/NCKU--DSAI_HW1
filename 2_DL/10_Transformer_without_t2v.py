...
from models import Transformer_01
    ...
    for i in range(50):
        ...
        if min_loss > test_loss:
            torch.save({ 
                ...
                'optimizer_state_dict': optimizer.state_dict()}, 'src/results/16_Transformer_02_epoch50_7days.pt')
            #np.savetxt("src/results/16_Transformer_02_pred_epoch50_7days.csv", np.asarray( pred_list ), delimiter=',')
            print(f"Predict: {pred_list}")

            np.savetxt("src/training_best/16_Transformer_02_train_epoch50_7days.csv", np.asarray( train_list ), delimiter=',')
