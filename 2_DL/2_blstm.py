...
try:
    for i in range(50):
        ...
        if t_best > total_loss:
            t_best = total_loss
            print("Train saved")
            np.savetxt("src/training_best/8_BLSTM_05_epoch50_7days.csv", np.asarray( train_list ), delimiter=',')
        ...
        if min_loss > test_loss:
            torch.save({ 
                ...
                'optimizer_state_dict': optimizer.state_dict()}, 'src/results/8_BLSTM_05_epoch50_7days.pt')
            np.savetxt("src/results/8_BLSTM_05_pred_epoch50_7days.csv", np.asarray( pred_list ), delimiter=',')
            #print(f"Predict: {pred_list}")

            np.savetxt("src/training_best/8_BLSTM_05_pred_epoch50_7days_pred.csv", np.asarray( pred_list ), delimiter=',')
